...
class ChatScreen extends StatefulWidget {
  ...
  ChatScreen({Key? key, required this.otheruserId, required this.curruserid})
      : super(key: key);

  @override
  State createState() =>
      ChatScreenState(otheruserId: otheruserId, curruserid: curruserid);
}

class ChatScreenState extends State<ChatScreen> {
  ChatScreenState(
      {Key? key, required this.otheruserId, required this.curruserid});
  ...
  readLocal() async {
    ...
    FirebaseFirestore.instance
        ...
        .doc(id)
        .update({'chattingWith': otheruserId});
  ...
  Widget buildItem(int index, DocumentSnapshot document) {
    if ((document.data() as Map)['idFrom'] == curruserid) {
      // Right (my message)
      return Row(
        children: <Widget>[
          (document.data() as Map)['type'] == 0
              // Text
              ? Container(
                  child: Text(
                    (document.data() as Map)['content'],
                    ...
                    child: Text(
                      DateFormat('dd MMM hh:mm a').format(
                          DateTime.fromMillisecondsSinceEpoch(int.parse(
                              (document.data() as Map)['timestamp']))),
                      style: TextStyle(
                          color: Colors.grey,
                          fontSize: 12.0,
  ...
  bool isLastMessageLeft(int index) {
    if ((index > 0 &&
            (listMessage[index - 1].data() as Map)['idFrom'] == curruserid) ||
        index == 0) {
      return true;
    } else {
      return false;
